...
function showMeme() {
  ...
  imageEle.setAttribute('src', randomMemeUrl);
...
function showJoke() {
  ...
  const jokeEle = document.createElement('p');
  jokeEle.textContent=randomJokeText;
  
  clearAll();
  
  document.querySelector('#joke-content').appendChild(jokeEle);
}

function showQuote() {
  const randomQuote = getRandomData('quotes');
  const quoteEle = document.createElement('p');
  const quoteEle2 = document.createElement('p');
  quoteEle.textContent=randomQuote.quote;
  quoteEle2.textContent='-'+randomQuote.author;

  clearAll();
  
  const container=document.querySelector('#quote-content');
  container.appendChild(quoteEle);
  container.appendChild(quoteEle2);
 }
...
function showRiddle() {
  ...
  answerEle.setAttribute('id','answer-content');  
...
function revealAnswers() {
  ...
  if(riddle && answer.hidden){
    ...
    alert('The answer is aready there!')  
  }else {
